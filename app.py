...
from var import (
    SP500_STOCKS, NASDAQ100_STOCKS, TRADE_HISTORY_TICKERS,
...
def load_market_pulse_data() -> dict:
    ...
    today = date.today()
    start = (today - timedelta(days=730 + 60)).strftime("%Y-%m-%d")
...
def get_period_dates(period, reference_date=None):
    ...
    period_map = {"1W": timedelta(weeks=1), "2W": timedelta(weeks=2), "1M": timedelta(days=30), "3M": timedelta(days=91), "6M": timedelta(days=182), "1Y": timedelta(days=365), "2Y": timedelta(days=730), "5Y": timedelta(days=1825)}
...
PERIOD_TO_FRAME_INTERVAL = {"1W": "D", "2W": "D", "1M": "W", "3M": "W", "6M": "ME", "YTD": "ME", "1Y": "ME", "2Y": "QE", "Custom": "W"}
...
with st.sidebar:
    ...
    page = st.radio("", ["📡 Signal Scanner", "📊 Market Pulse"], label_visibility="collapsed")
...
if page == "📡 Signal Scanner":
    with st.sidebar:
        ...
        scan_mode = st.selectbox("Scan Universe", ["Custom Watchlist", "S&P 500", "NASDAQ 100", "Trade History"])
        custom_input = ""
    ...
    if run_scan:
        raw_tickers = ([t.strip().upper() for t in custom_input.replace("\n", ",").split(",") if t.strip()] if scan_mode == "Custom Watchlist"
            else get_ticker_list("S&P 500") if scan_mode == "S&P 500" else get_ticker_list("NASDAQ 100") if scan_mode == "NASDAQ 100" else get_ticker_list("Trade History"))
        ...
        for i, ticker in enumerate(raw_tickers):
            ...
            sector_name = all_stock_map.get(ticker, ("Unknown", "Unknown"))[0]
            earn_display = f"{earn_date} ({earn_days}d)" if earn_date and earn_days is not None and earn_days >= 0 else (f"{earn_date} ({earn_days}d ago)" if earn_date and earn_days is not None else "—")
    ...
    if "scan_results" in st.session_state:
        ...
        def render_table(data):
            if data.empty: st.info("No signals."); return
            styled = (data.style.applymap(style_signal, subset=["Signal"]).applymap(style_check, subset=check_cols)
            ...
            st.dataframe(styled, use_container_width=True, height=min(600, 50 + len(data) * 36))
        with tab1: render_table(df_r.reset_index(drop=True))
        with tab2: render_table(buys.reset_index(drop=True))
        with tab3: render_table(holds.reset_index(drop=True))
        with tab4: render_table(sells.reset_index(drop=True))
        with tab5: render_table(blocked.reset_index(drop=True))
        st.markdown("---"); _, col_dl = st.columns([3, 1])
        with col_dl: st.download_button("⬇ CSV", df_r.to_csv(index=False), "scan_results.csv", "text/csv", use_container_width=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
    ...
    with st.spinner("📡 Loading 2 years of market data…"): _data = load_market_pulse_data()
    ...
    with st.sidebar:
        ...
        period_a = st.selectbox("Preset", ["1W", "2W", "1M", "3M", "6M", "YTD", "1Y", "2Y", "Custom"], index=0, key="period_a")
        if period_a == "Custom":
            start_a = st.date_input("Start", value=date.today()-timedelta(days=3), min_value=date.today()-timedelta(days=730), max_value=date.today(), key="start_a")
        ...
        if view_mode not in ["Single Period", "▶ Play Animation", "Sector Overview", "📈 Sector Trends"]:
            ...
            period_b = st.selectbox("Preset", ["1W", "2W", "1M", "3M", "6M", "YTD", "1Y", "2Y", "Custom"], index=2, key="period_b")
            if period_b == "Custom":
                start_b = st.date_input("Start", value=date.today()-timedelta(days=3), min_value=date.today()-timedelta(days=730), max_value=date.today(), key="start_b")
    ...
    if view_mode == "Sector Overview":
        ...
        st.plotly_chart(build_sector_heatmap(returns_a, market_caps, title=f"S&P 500 Sector Overview — {label_a}", color_range=color_range), use_container_width=True)
        ...
        if drill != "— select —":
            ...
            st.plotly_chart(build_heatmap(returns_a[[t for t in dt]], market_caps, title=f"{drill} — {label_a}", color_range=color_range), use_container_width=True)
    elif view_mode == "Single Period":
        label_a = period_a if period_a != "Custom" else f"{date_start_a} → {date_end_a}"; show_metrics(returns_a, label_a); st.markdown("---")
        st.plotly_chart(build_heatmap(returns_a, market_caps, title=f"S&P 500 — {label_a}", color_range=color_range), use_container_width=True)
        ...
        ca, cb = st.columns(2)
        with ca: st.markdown(f"#### A — {label_a}"); show_metrics(returns_a, label_a); st.plotly_chart(fn(returns_a, market_caps, title=f"A — {label_a}", color_range=color_range), use_container_width=True)
        with cb: st.markdown(f"#### B — {label_b}"); show_metrics(returns_b, label_b); st.plotly_chart(fn(returns_b, market_caps, title=f"B — {label_b}", color_range=color_range), use_container_width=True)
        ...
        if dl == "Sector":
            st.plotly_chart(build_sector_diff_heatmap(returns_a, returns_b, market_caps, title=f"Sector Δ: {label_b} vs {label_a}", color_range=color_range), use_container_width=True)
            ...
            st.dataframe(pd.DataFrame(sdrows), use_container_width=True, hide_index=True)
        else: st.plotly_chart(build_diff_heatmap(returns_a, returns_b, market_caps, title=f"Stock Δ: {label_b} vs {label_a}"), use_container_width=True)
        ...
        with st.spinner("⚙️ Pre-computing..."): st.plotly_chart(build_animated_heatmap(prices=prices, market_caps=market_caps, period=period_a if period_a != "Custom" else "Custom", start_date=date_start_a, end_date=date_end_a, color_range=color_range, selected_sectors=selected_sectors), use_container_width=True)
        ...
        for i in range(0, len(sectors_to_show), 4):
            ...
            for j, sector in enumerate(sectors_to_show[i:i+4]):
                ...
                if fig: cols[j].plotly_chart(fig, use_container_width=True)
    ...
    with st.expander("📋 Raw Returns Table", expanded=False):
        if not returns_a.empty:
            ...
            if view_mode == "Single Period":
                sel = st.dataframe(df_t, use_container_width=True, height=300, hide_index=True, on_select="rerun", selection_mode="single-row")
            ...
            else: st.dataframe(df_t, use_container_width=True, height=300, hide_index=True)
    st.markdown("---"); st.caption("⚠️ Data provided by Yahoo Finance for informational purposes only. Not financial advice.")
